// CopyTextContentsShellExt.cpp : Implementation of CCopyTextContentsShellExt

#include "stdafx.h"
#include "CopyTextContentsShellExt.h"
...
CCopyTextContentsShellExt::CCopyTextContentsShellExt()
{
...
STDMETHODIMP CCopyTextContentsShellExt::Initialize ( 
...
HRESULT CCopyTextContentsShellExt::QueryContextMenu (
...
// Insert the submenu into the ctx menu provided by Explorer.      
	...
	TCHAR prch[sizeof(_T("Software\\4dots Software\\CopyTextContents\\MenuItems"))]
	=_T("Software\\4dots Software\\CopyTextContents\\MenuItems");
...
//	lstrcat(prch,name);	
  ...
  //}
		...
		if ((dwErr == ERROR_SUCCESS && (lstrcmp(prcaption,_T("True"))==0) )
			|| dwErr!=ERROR_SUCCESS)
		{					
			InsertMenu ( hmenu, iCopyTextContents=upos++, MF_BYPOSITION, uID++, _T("Copy Text Contents"));				
		...
		if ((dwErr == ERROR_SUCCESS && (lstrcmp(prcaption2,_T("True"))==0))
			|| dwErr!=ERROR_SUCCESS) 
		{
			InsertMenu ( hmenu,iCopyTextContentsEncoding= upos++, MF_BYPOSITION, uID++, _T("Copy Text Contents with Encoding"));
			SetMenuItemBitmaps ( hmenu, upos-1, MF_BYPOSITION, m_hUnlockBmp, m_hUnlockBmp );
		}
...
HRESULT CCopyTextContentsShellExt::GetCommandString (
...
//	return E_FAIL;
  ...
  if (uFlags==GCS_VALIDATE)
  {
    ...
    LPCTSTR szText = _T("CopyTextContents");
...
HRESULT CCopyTextContentsShellExt::InvokeCommand(
...
lstrcatW(lpPathBuffer,_T("CopyTextContentstmp.txt"));
...
if (iVerb==iCopyTextContents)
{
lstrcatW(sParam,_T(" -CopyTextContents"));
}
else if (iVerb==iCopyTextContentsEncoding)
{
	lstrcatW(sParam,_T(" -CopyTextContentsEncoding"));
}
...
//lstrcatW(sParam,cmd);
	...
	//MessageBox ( NULL , sParam, _T("SimpleShlExt"),MB_ICONINFORMATION );	    
        ...
        lRet = reg.Open ( HKEY_LOCAL_MACHINE,
                          _T("Software\\4dots Software\\CopyTextContents"),KEY_QUERY_VALUE);
		...
		wcscat_s(szFp,_T("\\CopyTextContents.exe") );
